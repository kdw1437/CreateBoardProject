...
public class BoardDAO {
	...
	public void insertBoard(BoardBean bean) {
		...
		int ref = 0;//글 그룹을 의미 = 쿼리를 실행시켜서 가장 큰 ref값을 가져온 후 +1을 더해주면 된다.
	...
	public Vector<BoardBean> getAllBoard(int start, int end) {
		...
		try {
			...
			while(rs.next()) {
				//데이터를 패키징(가방 = Boardbean 클래스를 이용해서 패키징 해준다.)
	...
	public String getPass(int num) {
		...
		try {
			...
			//패스워드값을 저장
			if (rs.next()) {
				pass = rs.getString(1); //패스워드 값을 저장
			}
	...
	public int getAllCount() {
		...
		try {
			...
			rs = pstmt.executeQuery();
			if (rs.next()) {
				count = rs.getInt(1); //전체 게시글 수 return
			}
